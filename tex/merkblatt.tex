...
\subsubsection{Konvergenz in $\R^d$ und $\mathbb{C}$}
	...
	$$
	(komponentenweise Grenzwerte existieren).
...
\subsection{Stetigkeit: Rechnen mit Grenzwerten}
	...
	\end{tabular}
	\end{center}
	Summe, Differenz, Produkt und Quotient zweier konvergenter Folgen ist konvergent. Für
...
\section{Ableiten}
	...
	\Leftrightarrow \forall (x_n)_{n \in \N} \subset \I$ mit $x_n \to x_0$ folgt, dass\\
	$\frac{f(x_n) - f(x_0)}{x_n-x_0} = c \in \R
...
\subsection{Konvexit"at}
	...
	Anschaulich: Für 2 Punkte $x,y$ aus $D$, liegen alle Punkte auf der Geraden zwischen $x$ und
	$y$ wieder in $D$.\\
	Funktionen: $D \subseteq \R^n$ konvex \\
	$f: D \to \R$ konvex 	$\Leftrightarrow f(\lambda x + (1-\lambda)y) \leq \lambda f(x) + (1 - \lambda ) f(y)$\\
	$f(a,b) \to \R $ ist 2 mal stetig diffbar.  $f$ konvex $\Leftrightarrow f''(x) \geq 0$

\subsection{Mehrdimensionales Ableiten}
	Def.: $U \in \menge{R}^n$ offen, \quad $f:U \to \menge{R}^m$ \\
	$f$ heißt diffbar in $x \in U \Leftrightarrow \exists$ lin. Abb. 
	$A: U \to \menge{R}^m $\\
	$A \in \menge{R}^{m \times n}$ mit\\
...
\underline{Hauptsatz der Differential und Integralrechnung} \\
	Sei $f:[a,b] \rightarrow \menge{R} \textit{ stetig }, c \in [a,b]$ fest, dann \\
	$\quad F(x) := \int\limits_c^x f(y) \d y$
	\begin{enumerate}
	\itemsep-0.5em
	\item stetig diffbar
...
\subsubsection{"uber Quader $Q \in \R^n$}
	...
	Im Folgenden seien die Tr"ager aller Funktionen kompakt. \\
	Charakteristische Funktion $\chi_K$ einer kompakten Menge 
	$K \subset \R^n:
	\chi_K(x) = 
		\begin{cases}
			1, \text{ falls } x \in K \\ 
			0, \text{ falls } x \in \R^n \setminus K.\end{cases}$ \\
	Volumen von $K$:  \\
	$Vol(K) := \int\limits_{\R^n} \chi_K(x) \d^n x 
	\quad \in \R_+  \entspr \quad \int\limits_K 1 \ \d^n x$
\subsubsection{"uber beliebige stetige Funktionen mit kompakten Tr"ager}
	Sei $ \in \R^{n \times n}$ invertierbar, dann:\\ 
	$A\int\limits_{\R^n} f(Ax) |\det A| d^n x = \int\limits_{\R^n} f(y) d^n y$ \\
	Transformationssatz: Seien $U,V \subset \R^n$ offene Mengen und $\phi: U \to V$ stetig
	diffbar mit $\phi^{-1} : V \to U$ stetig diffbar.
...
\section{Potenzreihe}
	...
	dann heißt $f:(-r,+r) \rightarrow \menge{R} \quad f(x) 
	= \sum\limits_{k=0}^\infty a_kx^k$ Potenzreihe. \\
	$r > 0$ heißt Konvergenzradius, $r = \frac{1}{\limsup \sqrt[n]{|a_n|}}$. \\
	Welche Funktionen $f$ haben die Darstellung:\\
	$f(x) = \sum\limits_{k=0}^\infty a_kx^k \quad \forall x \in (-r,r) \rightarrow$
	\underline{Taylorentwicklung} \\
	$f: \menge{I} \rightarrow \menge{R}, \menge{I} = (a,b) \quad f$ sei $(n+1)$-mal stetig diffbar, dann: \\
...
\subsection{Kr"ummung}
	...
	Falls $\gamma = (x,y)$, gilt
	$$K(t) 
	= \frac{\dot{x}\ddot{y} - \dot{y}\ddot{x}}{\sqrt{\dot{x}^2 + \dot{y}^2}^{\; 3}} (t)$$
	Falls $\gamma = (x, f(x))$ gilt 
	$K(x) = \frac{f''(x)}{\sqrt{1+f'^2(x)}^{\; 3}}$
...
$$
$$
	\qquad \int \frac{1}{x} \, \d x = \ln |x| + C, \qquad
...
\subsection{Sonstiges}
	...
	\includegraphics[scale=0.2]{close_enough}
