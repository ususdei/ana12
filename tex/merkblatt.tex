...
\section{Reihen}
	...
	\subsection{Konvergenzkriterien}
		...
		\subsubsection{Leibnizsches Konvergenzkriterium}
			Sei $(a_n) \subset \R$ eine \textit{monoton fallende Nullfolge}. Dann konvergiert
			die zugehörige alternierende Reihe
			$ \sum\limits_{k=0}^{\infty} (-1)^k a_k = s $ und die Partialsumme $s_n$
			approximiert den Reihenwert $s$ bis auf einen Fehler, der höchstens so groß ist,
			wie der Betrag des ersten, weggelassenen Summanden.
			$ \left| \sum\limits_{k=n+1}^{\infty} (-1)^k a_k  \right| \leq a_{n+1} $
...
\subsection{Vergleichskriterien die zweite xD}
	\subsubsection{Majorantenkriterium}
		Seien $( \sum\limits_{k=1}^n a_k) \in \C$ und $( \sum\limits_{k=1}^n b_k) \in \R$ Reihen mit $|a_k| \leq b_k$.\\
		$( \sum\limits_{k=1}^n b_k)$ heißt Majorante von $( \sum\limits_{k=1}^n a_k)$ und es gilt:
		\begin{itemize}
		\item $( \sum\limits_{k=1}^n b_k)$ konvergiert  $\Rightarrow ( \sum\limits_{k=1}^n a_k)$ konvergiert
		\item $( \sum\limits_{k=1}^n a_k)$ divergiert $\Rightarrow ( \sum\limits_{k=1}^n b_k)$ divergiert
		\end{itemize}
	\subsubsection{Quotientenkriterium}
		$( \sum\limits_{k=1}^n a_k) \in \C$ und $a_k \neq 0 \quad \forall k \leq k_0$ mit Grenzwert $ q:= \lim_{k \to \infty} |\frac{a_{k+1}}{a_k}|$
		\begin{itemize}
		...
		\end{itemize}
	\subsubsection{Wurzelkriterium}
	$(\sum\limits_{k=1}^n a_k)$ mit $\rho := \limsup_{k \to \infty} \sqrt[k]{|a_n|} $
	\begin{itemize}
	...
	\end{itemize}
	\subsubsection{Leibnizsches Konvergenzkriterium}
		$(a_n) \in R$ und $(a_n)$ ist monoton fallende Nullfolge $ \Rightarrow \sum\limits_{k=0}^ \infty (-1)^k a_k = s \quad$ konvergiert
